...
# CODE COMPLETO COMUNICAZIONE - TELECAMERA
# CODE RILEVA COLORE ROSSO E LO INVIA AD ARDUINO CHE RISPONDE
# BISOGNA RIVEDERE I PARAMETRI PERCHE NON RILEVA BENE IL ROSSO
# MEGLIO INVIARE STRINGHE DI 1, 2 CARATTERI DA ARDUINO
...
while True:
    ...
    if colore == "RED":
        ...
        msg = receive()
        if msg == "y":
            print("PRENDILOOOOOOOOOO")
            while True:
                print("rosso")  # Esci dal ciclo principale se il messaggio e "y"
    else:  # Se il colore non e "RED", invia "altro"
        while True:
            send(b"nulla\n")
            msg = receive()
            if msg == "n":
                print("pass")
                break  # Esci dal ciclo interno, non usare return, perche il ciclo principale continua
